...
import { fauxGoogle, DOSSIER_MIME, f, d, compter } from './outils/faux-google.mjs';
...
/* ═══════════════════════════════════════════════════════════════════════════
   Le faux Google et le compteur de coût vivent dans `tests/outils/faux-google.mjs`
   depuis le 19/09/2026 : le banc de coût du passage a besoin EXACTEMENT du même
   serveur, et deux copies d'un faux serveur finiraient par diverger.
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ── LES DEUX FORMES DE `captions` — la contradiction des documents 31 et 37 ──
   Le 31 décrit un objet qui désigne un fichier du dossier ; le 37 montre une
   chaîne qui EST la légende. ChatGPT a suivi le 37, et la lecture ne
   connaissait que le 31 : une publication prévue ne pouvait pas partir. Le
   lecteur admet désormais les deux, et NE RÉPARE toujours rien. */

/** Le même Drive, mais avec les légendes posées EN LIGNE dans le manifeste. */
function driveLegendeEnLigne(captions) {
  return fauxGoogle({
    listes: {
      [DOSSIER]: [d('w1', 'WEEK_21-27Sept')],
      w1: [d('j1', 'Lundi_21_Sept')],
    ...
    fichiers: { 'f-json': JSON.stringify(manifeste({ captions })) },
  });
}

test('🔴 une légende EN LIGNE est rendue telle quelle, SANS aucun téléchargement', async () => {
  viderCacheJeton();
  const texte = '🎒 Rentrée : commençons par une liste claire\n\nFamilles, écoles…';
  const g = driveLegendeEnLigne({ facebook: texte });
  const lot = await creerClientDrive({ env: env(), fetchImpl: g.impl }).lirePublications();

  assert.equal(lot.publications.length, 1, JSON.stringify(lot.ecartees));
  const cap = lot.publications[0].captions.facebook;
  assert.equal(cap.texte, texte, 'le texte doit arriver au caractère près');
  assert.equal(cap.chemin_relatif, null, 'il n y a aucun fichier à chercher');
  assert.equal(cap.fichier_id, null);
  // Seul le manifeste est téléchargé : la légende, elle, était dedans.
  assert.equal(g.appels.filter((a) => /alt=media/.test(a.url)).length, 1);
});

test('⛔ une légende EN LIGNE inutilisable est DITE invalide, pas prise pour absente', async () => {
  for (const fantome of ['undefined', 'null', 'None', '', '   ']) {
    viderCacheJeton();
    const g = driveLegendeEnLigne({ facebook: fantome });
    const lot = await creerClientDrive({ env: env(), fetchImpl: g.impl }).lirePublications();

    const cap = lot.publications[0].captions.facebook;
    assert.equal(cap.texte, null, `« ${fantome} » ne doit jamais devenir une légende`);
    assert.ok(cap.invalide, `« ${fantome} » doit être signalé comme inutilisable`);
    assert.match(lot.publications[0].avertissements.join(' '), /inutilisable/);
  }
});

test('⛔ un chemin_relatif valant « undefined » est refusé AVANT d aller le chercher', async () => {
  viderCacheJeton();
  const g = driveLegendeEnLigne({ facebook: { chemin_relatif: 'undefined' } });
  const lot = await creerClientDrive({ env: env(), fetchImpl: g.impl }).lirePublications();

  const cap = lot.publications[0].captions.facebook;
  assert.equal(cap.chemin_relatif, null, '« undefined » n est pas un nom de fichier');
  assert.match(cap.invalide, /undefined/);
});

test('les DEUX formes cohabitent dans le même manifeste', async () => {
  viderCacheJeton();
  const g = fauxGoogle({
    listes: {
      [DOSSIER]: [d('w1', 'WEEK_21-27Sept')],
      ...
      p1: [
        f('f-json', 'publication.json', 'application/json'),
        f('f-media', MEDIA),
        f('f-cap', 'caption_instagram.txt', 'text/plain'),
      ],
    },
    fichiers: {
      'f-json': JSON.stringify(manifeste({
        captions: {
          facebook: 'Texte posé directement dans le manifeste.',
          instagram: { chemin_relatif: 'caption_instagram.txt' },
        },
      })),
      'f-cap': 'Texte dans un fichier du dossier.',
    },
  });
  const lot = await creerClientDrive({ env: env(), fetchImpl: g.impl }).lirePublications();

  const caps = lot.publications[0].captions;
  assert.equal(caps.facebook.texte, 'Texte posé directement dans le manifeste.');
  assert.equal(caps.instagram.fichier_id, 'f-cap');
  assert.equal(caps.instagram.texte, null);
});
